# 爱数AnyShare云盘 start_service 远程代码执行漏洞 POC (CVE-2025-34160)
...
**影响产品**：爱数AnyShare智能内容管理平台（2025年8月前发布版本）
...
爱数AnyShare云盘系统的 `/api/ServiceAgent/start_service` 接口存在命令注入漏洞。攻击者可通过构造恶意JSON数组，利用反引号执行任意系统命令。未经身份验证的远程攻击者可借此完全控制目标服务器。

本POC通过发送包含 `` `sleep 5` `` 的JSON数据，根据响应延迟判断漏洞是否存在（无害检测）。
...
参数	说明
-h, --help	显示帮助信息
-u URL, --url URL	检测单个 URL（只需提供域名/IP，如 http://192.168.1.100）
...
text
[+] http://192.168.1.100/api/ServiceAgent/start_service 存在漏洞 (响应延迟: 5.12s)
[-] http://192.168.1.101/api/ServiceAgent/start_service 不存在漏洞 (响应延迟: 0.03s)
[!] 请求 http://192.168.1.102/api/ServiceAgent/start_service 时发生错误: Connection refused
